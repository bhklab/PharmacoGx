...
grRegression <- function(
  ...
  #If true, fits parameters to a transformed GR curve
  ...
  GEC50 <- 10^Hill_fit[3] *
    ((2^tau - (1 + Ginf)^tau) / ((1 + Ginf)^tau - (2 * Ginf)^tau))^(1 /
      Hill_fit[1])
  GHS <- (1 - Hill_fit[2]) /
    tau *
    (.Hill(log10(GEC50), Hill_fit))^(1 / tau - 1) *
    1 /
    (1 + (GEC50 / 10^Hill_fit[3])^Hill_fit[1])^2 *
    Hill_fit[1] /
    10^Hill_fit[3] *
    (GEC50 / 10^Hill_fit[3])^(Hill_fit[1] - 1) *
    GEC50 *
    log(10)
  ...
  return(list(
    GHS = as.numeric(GHS),
    Ginf = as.numeric(Ginf),
    GEC50 = as.numeric(GEC50)
  ))
}
