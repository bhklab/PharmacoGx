...
intersectPSet <- function(
  ...
  if (length(pSets) > 1) {
    ...
    if ("cell.lines" %in% intersectOn) {
      common.cells <- .intersectList(lapply(pSets, function(x) {
        return(sampleNames(x))
      }))
      if (!missing(cells)) {
    ...
    if (("drugs" %in% intersectOn) & ("cell.lines" %in% intersectOn)) {
      common.exps <- .intersectList(lapply(pSets, function(x) {
        if (
          "sampleid" %in%
            colnames(sensitivityInfo(x)) &
            "treatmentid" %in% colnames(sensitivityInfo(x))
        ) {
          paste(
            sensitivityInfo(x)$sampleid,
            sensitivityInfo(x)$treatmentid,
            sep = "_"
          )
        } else {
          NULL
      ...
      if (strictIntersect) {
        ...
        expMatch <- lapply(expMatch, function(x) {
          names(x) <- x
          return(x)
        })
      }
    ...
    if (
      ...
      pSets <- .calculateSensitivitiesStar(
        pSets,
        exps = expMatch,
        cap = 100,
        nthread = nthread
                ...
                return(.intersectList(lapply(SEs, function(SE) {
                  if (
                    length(grep(
                      molecular.type,
                      S4Vectors::metadata(
                        molecularProfilesSlot(pSet)[[SE]]
                      )$annotation
                    )) >
                      0
                  ) {
                    intersect(
                      colData(molecularProfilesSlot(pSet)[[SE]])$sampleid,
                      common.cells
                    )
                  }
                })))
            ...
            .intersectList(lapply(pSets, function(pSet) {
              ...
              return(CoreGx::.unionList(lapply(SEs, function(SE) {
                if (
                  length(grep(
                    molecular.type,
                    S4Vectors::metadata(molecularProfilesSlot(pSet)[[
                      SE
                    ]])$annotation
                  )) >
                    0
                  ...
                  intersect(
                    SummarizedExperiment::colData(molecularProfilesSlot(pSet)[[
                      SE
                    ]])$sampleid,
                    common.cells
                  )
      ...
      if (("drugs" %in% intersectOn) & ("cell.lines" %in% intersectOn)) {
        if (strictIntersect) {
          pSets[[i]] <- subsetTo(
            ...
            cells = common.cells,
            exps = expMatch,
            molecular.data.cells = common.molecular.cells
          )
        } else {
          pSets[[i]] <- subsetTo(
            pSets[[i]],
