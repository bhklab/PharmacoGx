...
rankGeneDrugPerturbation <-
  function(
    ...
    if (
      ...
      stop(
        "length of drug.id, drug.concentration, type, xp, duration and batch should be equal to the number of rows of data!"
      )
    }
    names(drug.id) <- names(drug.concentration) <- names(type) <- names(
      batch
    ) <- names(duration) <- rownames(data)
    ...
    for (bb in seq_len(length(ubatch))) {
      ...
      xpix <- rownames(data)[
        complete.cases(batch, xp) & batch == ubatch[bb] & xp == "perturbation"
      ]
      ctrlix <- rownames(data)[
        complete.cases(batch, xp) & batch == ubatch[bb] & xp == "control"
      ]

      if (
        all(!is.na(c(xpix, ctrlix))) && length(xpix) > 0 && length(ctrlix) > 0
      ) {
    ...
    for (ll in seq_len(length(ltype))) {
      ...
      if (
        ...
        rest <- matrix(
          ...
          nrow = nrow(data),
          ncol = length(nc),
          dimnames = list(rownames(data), nc)
        )
      } else {
        ...
        if (nthread > 1) {
          ...
          splitix <- splitix[
            vapply(splitix, length, FUN.VALUE = numeric(1)) > 0
          ]
