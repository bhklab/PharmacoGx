...
geneDrugSensitivity <- function(
  ...
  if (ncol(drugpheno) > 1) {
    ff0 <- paste(
      "cbind(",
      paste(
        paste("drugpheno", seq_len(ncol(drugpheno)), sep = "."),
        collapse = ","
      ),
      ")",
      sep = ""
  ...
  if (
    ...
    if (any(unlist(lapply(drugpheno, is.factor)))) {
      ...
      rest <- c(
        "estimate" = rr$coefficients[
          grep("^x", rownames(rr$coefficients)),
          "Estimate"
        ],
        "se" = rr$coefficients[
          grep("^x", rownames(rr$coefficients)),
      ...
      if (ncol(drugpheno) > 1) {
        ...
        rest <- lapply(seq_len(ncol(drugpheno)), function(i) {
          ...
          rest <- c(
            rr[[i]]$coefficients[
              grep("^x", rownames(rr[[i]]$coefficients)),
              "Estimate"
            ],
            rr[[i]]$coefficients[
              grep("^x", rownames(rr[[i]]$coefficients)),
        ...
        rest <- c(
          rest,
          "n" = nn,
          "fstat" = rrc$stats[grep("^x", rownames(rrc$stats)), "approx F"],
          "pvalue" = rrc$stats[grep("^x", rownames(rrc$stats)), "Pr(>F)"]
        )
      } else {
        rrc <- stats::anova(rr0, rr1, test = "F")
        if (
          !length(rr$coefficients[
            grep("^x", rownames(rr$coefficients)),
            "Estimate"
          ])
        ) {
          stop(
            "A model failed to converge even with sufficient data. Please investigate further"
          )
        }
        rest <- c(
          "estimate" = rr$coefficients[
    ...
    ## add model
    if (model) {
      rest <- list("stats" = rest, "model" = rr1)
    }
  }
  return(rest)
}
