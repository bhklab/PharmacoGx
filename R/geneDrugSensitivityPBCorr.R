...
geneDrugSensitivityPBCorr <- function(
  x,
  type,
  batch,
  drugpheno,
  ...
  if (!is.factor(dd[[2]])) {
    stop(
      "Molecular Feature is not discrete, but point biserial correlation was requested"
    )
    ...
    warning(
      "Some features had less than 3 observations per category, returning NA."
    )
    return(rest)
  } else {
    ...
    if (test == "resampling") {
      ...
      if (ncol(dd) > 2) {
        ...
        if (ncol(dd) == 3) {
          sample_function <- function() {
            ...
            for (gp in unique(dd[, 3])) {
              partial.x[dd[, 3] == gp] <- partial.x[dd[, 3] == gp] -
                mean(partial.x[dd[, 3] == gp])
              partial.dp[dd[, 3] == gp] <- partial.dp[dd[, 3] == gp] -
                mean(partial.dp[dd[, 3] == gp])
            }
        ...
        pcor.boot <- function(ddd, w) {
          ...
          ddd[, -c(1, 2)] <- ddd[, -c(1, 2), drop = FALSE][,
            apply(ddd[, -c(1, 2), drop = FALSE], 2, function(x) {
          ...
          if (ncol(ddd) == 3) {
            ...
            for (gp in unique(ddd[, 3])) {
              partial.x[ddd[, 3] == gp] <- partial.x[ddd[, 3] == gp] -
                mean(partial.x[ddd[, 3] == gp])
              partial.dp[ddd[, 3] == gp] <- partial.dp[ddd[, 3] == gp] -
                mean(partial.dp[ddd[, 3] == gp])
            }
          } else if (ncol(ddd) == 2) {
            partial.dp <- ddd[, 1]
            partial.x <- ddd[, 2]
        ...
        cint <- tryCatch(
          boot.ci(boot.out, conf = conf.level, type = "bca")$bca[, 4:5],
          error = function(e) {
        ...
        cint <- tryCatch(
          ...
          error = function(e) {
            if (
              e$message == "estimated adjustment 'w' is infinite" ||
                e$message ==
      ...
      p.value <- 2 *
        min(pt(stat, df = df), pt(stat, df = df, lower.tail = FALSE))
