...
filterNoisyCurves <- function(
  ...
  acceptable <- mclapply(
    rownames(sensitivityInfo(pSet)),
    function(xp) {
      #for(xp in rownames(sensitivityInfo(pSet))){
      drug.responses <- as.data.frame(
        apply(sensitivityRaw(pSet)[xp, , ], 2, as.numeric),
...
.computeDelta <- function(xx, trunc = TRUE) {
  ...
  if (trunc) {
    return(c(
      pmin(100, xx[seq(2, length(xx))]) - pmin(100, xx[seq_along(xx) - 1]),
      0
    ))
  } else {
