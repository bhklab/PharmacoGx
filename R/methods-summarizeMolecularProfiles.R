...
setMethod(
  ...
  function(
    ...
    if (!(mDataType %in% mDataTypes)) {
      stop(sprintf(
        "Invalid mDataType, choose among: %s",
    ...
    if (missing(features)) {
      ...
      if (verbose && !all(fix)) {
        warning(sprintf(
          "Only %i/%i features can be found",
          sum(fix),
          length(features)
        ))
      }
    ...
    if (datasetType(object) %in% c("perturbation", "both") && removeTreated) {
      if (!"xptype" %in% colnames(phenoInfo(object, mDataType))) {
        ...
        keepCols <- phenoInfo(object, mDataType)$xptype %in%
          c("control", "untreated")
        molecularProfilesSlot(object)[[mDataType]] <- molecularProfilesSlot(
          object
        )[[mDataType]][, keepCols]
      }
    }
    ...
    if (any(colnames(dd) != rownames(pp))) {
      warning(
        "Samples in phenodata and expression matrices must be ordered the same way"
      )
      dd <- dd[, rownames(pp), drop = FALSE]
    }
    if (!fill.missing) {
      cell.lines <- intersect(
        cell.lines,
        unique(pp[!is.na(pp[, "sampleid"]), "sampleid"])
      )
    }
    ...
    duplix <- unique(pp[
      !is.na(pp[, "sampleid"]) & duplicated(pp[, "sampleid"]),
      "sampleid"
    ])
    ...
    if (length(duplix) > 0) {
      if (verbose) {
        message(sprintf(
          "Summarizing %s molecular data for:\t%s",
          mDataType,
      ...
      for (x in duplix) {
        ...
        switch(
          ...
          "and" = {
            ddt <- apply(dd[, myx, drop = FALSE], 1, function(x) {
              do.call(`&`, as.list(x))
            })
          },
          "or" = {
    ...
    res <- res[features, ]
    S4Vectors::metadata(res) <- S4Vectors::metadata(molecularProfilesSlot(
      object
    )[[mDataType]])
    return(res)
  }
