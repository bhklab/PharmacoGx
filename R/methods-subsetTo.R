...
.subsetToPharmacoSet <- function(
  ...
  molecularProfilesSlot(object) <- lapply(
    molecularProfilesSlot(object),
    function(SE, cells, drugs, molecular.data.cells) {
      molecular.data.type <- ifelse(
      ...
      if (length(cells) != 0) {
        ...
        cell_line_index <- which(
          SummarizedExperiment::colData(SE)[["sampleid"]] %in% cells
        )
      ...
      drugs_index <- NULL
      if (
        datasetType(object) == 'perturbation' || datasetType(object) == 'both'
      ) {
        if (length(drugs) != 0) {
          ...
          drugs_index <- which(
            SummarizedExperiment::colData(SE)[["treatmentid"]] %in% drugs
          )
          ...
          if (keep.controls) {
            control_indices <- which(
              SummarizedExperiment::colData(SE)[['xptype']] == 'control'
            )
  ...
  if (
    ...
    sensitivityProfiles(object) <- sensitivityProfiles(object)[
      exps,
      ,
      drop = drop
    ]
    ...
    treatmentResponse(object)[names(treatmentResponse(object))[
      names(treatmentResponse(object)) != 'n'
    ]] <- lapply(
      treatmentResponse(object)[names(treatmentResponse(object))[
        names(treatmentResponse(object)) != 'n'
      ]],
  ...
  if (
    datasetType(object) == 'sensitivity' |
      datasetType(object) == 'both' & length(exps) == 0
  ) {
