sanitizeInput <- function(
  ...
  if (!missing(Hill_fit) && missing(viability)) {
    ...
    if (conc_as_log == FALSE && Hill_fit[[3]] < 0) {
      ...
      print(Hill_fit[[3]])
      stop(
        "'conc_as_log' flag may be set incorrectly, as the EC50 is negative when positive value is expected."
      )
    }
    ...
    if (missing(conc)) {
      ...
      if (prod(is.finite(conc)) != 1) {
        print(conc)
        stop(
          "Concentration vector contains elements which are not real numbers."
        )
      }
      if (conc_as_log == FALSE && min(conc) < 0) {
        print(conc)
