...
test_that("Sensitivity result did not change since last time", {
	data(GDSCsmall)
  drug.sensitivity <- drugSensitivitySig(GDSCsmall, mDataType="rna", nthread=1, features = fNames(GDSCsmall, "rna")[1:10])
	expect_equal_to_reference(drug.sensitivity@.Data, "drug.sensitivityGDSCSmall.rds")
	drug.sensitivity <- drugSensitivitySig(GDSCsmall, mDataType="rna", nthread=1, features = fNames(GDSCsmall, "rna")[1:10], sensitivity.cutoff = 0.2, sensitivity.measure="auc_recomputed")
