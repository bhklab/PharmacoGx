...
test_that("Sensitivity result did not change since last time", {
	...
	drug.sensitivity <- drugSensitivitySig(GDSCsmall, mDataType="rna", nthread=1, features = fNames(GDSCsmall, "rna")[1:25], sensitivity.cutoff = 0.2, sensitivity.measure="auc_recomputed")
