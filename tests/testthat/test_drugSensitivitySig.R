...
test_that("Perturbation result did not change since last time", {
	...
	drug.sensitivity <- drugSensitivitySig(GDSCsmall, mDataType="rna", nthread=1, features = fNames(GDSCsmall, "rna")[1:50], sensitivity.cutoff = 0.1, sensitivity.measure="auc_recomputed")
	expect_equal_to_reference(drug.sensitivity@.Data, "drug.sensitivity.discreteGDSCSmall.rds")
