...
test_that("summarizeSensitivityProfiles produces correct values.", {
  ...
  testCells <- sensitivityProfiles(GDSCsmall2)[
    order(sensitivityInfo(GDSCsmall2)$sampleid),
    "auc_recomputed",
    drop = FALSE
  ]

  testSummary <- summarizeSensitivityProfiles(
    GDSCsmall2,
    summary.stat = "median",
    fill.missing = FALSE
  )
  ...
  expect_equivalent(
    ...
    mapply(
      ...
      testCells[seq(1, 18, 2), ],
      testCells[seq(1, 18, 2) + 1, ]
    )
  )
...
test_that("Summarize Sensitivity Profiles parameters work as expected", {
  ...
  expect_equal(
    ncol(summarizeSensitivityProfiles(GDSCsmall, fill.missing = FALSE)),
    length(unique(sensitivityInfo(GDSCsmall)$sampleid))
  )
  expect_equal(
    ncol(summarizeSensitivityProfiles(GDSCsmall, fill.missing = TRUE)),
    length(sampleNames(GDSCsmall))
  )
})
