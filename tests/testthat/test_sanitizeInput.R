...
test_that("Function sanitizeInput yells at user sufficiently.", {
  expect_error(PharmacoGx:::sanitizeInput(
  ...
  expect_warning(
    ...
    "'viability_as_pct' flag may be set incorrectly"
  )
})
...
test_that("Function sanitizeInput returns correct values.", {
  expect_equal(
    ...
    list(
      ...
      viability = c(100, 90, 80, 70, 60, 50, 40) / 100
    )
  )
  expect_equal(
    PharmacoGx:::sanitizeInput(
    ...
    list(log_conc = log10(c(1, 2, 3)), viability = c(90, 80, 70) / 100)
  )
  expect_equal(
    PharmacoGx:::sanitizeInput(
      conc = c(0, 1, 2, 3),
