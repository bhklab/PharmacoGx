...
test_that("Summarize Molecular Profiles fails gracefully.", {
  ...
  expect_error(
    ...
    "argument \"mDataType\" is missing"
  )
  expect_error(
    summarizeMolecularProfiles(GDSCsmall, "rnaseq"),
    "Invalid mDataType"
  )
})
...
test_that("Summarize Molecular Profiles correctly summarizes replicates", {
  ...
  testCells <- SummarizedExperiment::assay(
    molecularProfilesSlot(GDSCsmall)$rna,
    1
  )[, myx]
  testSummary <- summarizeMolecularProfiles(
    GDSCsmall,
    "rna",
    summary.stat = "median"
  )
  ...
  testCells <- SummarizedExperiment::assay(
    molecularProfilesSlot(GDSCsmall2)$mutation,
    1
  )
  ...
  expect_equal(
    sum(as.numeric(SummarizedExperiment::assay(testSummary, 1)), na.rm = TRUE),
    2
  )
...
test_that("Summarize Molecular Profiles parameters work as expected", {
  expect_equal(
    summarizeMolecularProfiles(GDSCsmall, "rna", summarize = FALSE),
    molecularProfilesSlot(GDSCsmall)$rna
  )
  expect_silent(summarizeMolecularProfiles(GDSCsmall, "rna", verbose = FALSE))
  GDSCsmall2 <- GDSCsmall
  molecularProfilesSlot(GDSCsmall2)$rna <- molecularProfilesSlot(
    GDSCsmall2
  )$rna[, 1]
  expect_equivalent(
    ncol(summarizeMolecularProfiles(GDSCsmall2, "rna", fill.missing = FALSE)),
    1
  )
  expect_equivalent(
    ncol(summarizeMolecularProfiles(GDSCsmall2, "rna", fill.missing = TRUE)),
    length(sampleNames(GDSCsmall2))
  )
})
