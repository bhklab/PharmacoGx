...
for (cutoff in Rsq_cut_off) {
    ...
    spearman <- c(spearman,
        df_[, cor(Bliss_score, zip_delta, use="complete", method="spearman")]
    )
    n_drugs <- c(n_drugs, df_[, uniqueN(.SD[, .(treatment1id, treatment2id)])])
}
spearman_df <- data.frame(Rsq_cut_off, spearman, n_drugs)
```


```{r}
spearman_df |>
    ggplot(aes(x=Rsq_cut_off, y=spearman, label=n_drugs)) +
    geom_point() +
    geom_text(angle=45, vjust=-0.5) +
    geom_smooth() +
    ggtitle("Spearman for Bliss score vs ZIP delta")
```
