...
For LongTable-backed sensitivity data, `summarizeSensitivityProfiles()` first
uses `treatmentid` when it is available. If a dataset instead stores
monotherapy treatments in `treatment1id`, the function now falls back to that
column and ignores rows with a non-empty `treatment2id`. Combination response
summaries still require an explicit sensitivity matrix, and atypical profile
columns may require an explicit `sensitivity.measure` such as `"SCORE"`.
...
*PharmacoGx* includes functions to calculate estimated AAC (Area Above
response Curve), AUC (Area Under the viability Curve) and IC50 values from
drugs dose response experiments that
...
with the original data. The historical sensitivity summaries are labelled as
*auc\_published* or *auc\_recomputed* in older objects, while new recomputed
values should be interpreted as AAC sensitivity summaries.

While the *PharmacoSets* already contain the recomputed sensitivity data, the
AUC, AAC and IC50 can be calculated for arbitrary data using the
*computeIC50*, *computeAUC* and *computeAAC* functions. Both area summaries can
be calculated using either the area under the actual points recorded, or the
area under the fitted curve.


## Curve Fitting

While AUC and AAC can be numerically calculated without curve fitting, to estimate the
